...
public abstract class AbstractHibernateDao<T> implements DaoInterface<T> {
	...
	public List<T> getAllByOrder(String orderBy, String orderDir) {
		this.logger.debug("getting all " + getTableName() + " by order: "
				+ orderBy + " / " + orderDir);
	...
	public int getCount() {
		...
		return this.simpleJdbcTemplate.queryForInt("Select count(*) from "
				+ getTableName());
	}
	...
	public void saveOrUpdateAll(Collection<T> entities) {
		logger.debug("Saving/Updating all " + entities.size() + " "
	...
	public T load(Serializable id) {
		logger.debug("Loading " + persistentClass.getSimpleName()
				+ " with id: " + id);
