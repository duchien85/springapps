...
<link type="text/css" rel="stylesheet"
  href="<c:url value='/yui_2.6.0/build/autocomplete/assets/skins/sam/autocomplete.css'/>">
...
<script type="text/javascript" src="<c:url value='/yui_2.6.0/build/datasource/datasource-min.js'/>"></script>
<script type="text/javascript" src="<c:url value='/yui_2.6.0/build/get/get-min.js'/>"></script>
<!-- OPTIONAL: Connection (required only if using XHRDataSource) -->
<script type="text/javascript" src="<c:url value='/yui_2.6.0/build/connection/connection-min.js'/>"></script>
<!-- OPTIONAL: Animation (required only if enabling animation) -->
<script type="text/javascript" src="<c:url value='/yui_2.6.0/build/animation/animation-min.js'/>"></script>
<!-- OPTIONAL: JSON (enables JSON validation) -->
<script type="text/javascript" src="<c:url value='yui_2.6.0/build/json/json-min.js'/>"></script>
<!-- Source file -->
<script type="text/javascript" src="<c:url value='/yui_2.6.0/build/autocomplete/autocomplete-min.js'/>"></script>
...
<form:form method="post" action="doSearch.htm" modelAttribute="widgetSearchModel" id="searchForm">
  <table class="formTable">
    <tr>
      <td class="form_label_left"><form:label path="name" cssErrorClass="errors">Name</form:label>&nbsp; <span
        style="display: none" id="waitImageSpan"><img src="<c:url value='/images/wait.gif'/>" id="waitImage"
        alt="Calendar" /></span></td>
    </tr>
    <tr>
      <td class="form_input">
      <div id="myAutoComplete"><form:input path="name" size="20" /> <br />
      <br />
    ...
    <tr>
      ...
      <table class="formTable">
        <tr>
          ...
          <button type="button" id="calButtonBegin" title="Show Calendar"><img
            src="<c:url value='/images/calbtn.gif'/>" alt="Calendar" /></button>
        ...
        <tr>
          ...
          <button type="button" id="calButtonEnd" title="Show Calendar"><img
            src="<c:url value='/images/calbtn.gif'/>" alt="Calendar" /></button>
          <div id="containerEnd">
          <div class="hd">Calendar</div>
          <div class="bd">
    ...
    <tr>
      <td class="form_input"><form:select path="cool">
  ...
  <button type="button" id="callWS" title=""><img src="<c:url value='/images/calbtn.gif'/>" width="18"
    ...
    <tbody>
      <c:forEach items="${widgets.data}" var="w" varStatus="st">
        <tr class="${st.index % 2 == 0 ? 'odd' : 'even'}">
          ...
          <td>${w.cool}</td>
          <td><a href="<c:url value="/widget/search/edit.htm"/>?widgetId=${w.id}"><img
            src="<c:url value="/images/Sweetie/png-24/16-tag-pencil.png" />" /></a></td>
          <td><a href="<c:url value="/widget/search/delete.htm"/>?widgetId=${w.id}"><img
            src="<c:url value="/images/Sweetie/png-24/16-em-cross.png" />" /></a></td>
        </tr>
      </c:forEach>
    </tbody>
  </table>
...
<script type="text/javascript">
	var autoCompletefunc = function() {
		...
		ac.dataErrorEvent.subscribe(function() {
			$('waitImageSpan').hide();
		});
...
<script>
	YAHOO.util.Event.onDOMReady(function() {
		...
		calendarBegin = new YAHOO.widget.Calendar("calBegin", {
			iframe : true,
			hide_blank_weeks : true
		});
		calendarBegin.inputField = YAHOO.util.Dom.get("beginInitialTime");

		calendarEnd = new YAHOO.widget.Calendar("calEnd", {
			iframe : true,
			hide_blank_weeks : true
		});
