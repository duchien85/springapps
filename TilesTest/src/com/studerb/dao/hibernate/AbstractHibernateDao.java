...
public abstract class AbstractHibernateDao<T> implements DaoInterface<T> {
	...
	@Override
	public List<T> getJmesa(TableFacade tableFacade) {
		Limit limit = tableFacade.getLimit();
		Criteria criteria = getCurrentSession().createCriteria(persistentClass);
		int maxResults = limit.getRowSelect().getRowEnd() - limit.getRowSelect().getRowStart() + 1;
		criteria.setFirstResult(limit.getRowSelect().getRowStart());
		criteria.setMaxResults(maxResults > 0 ? maxResults : 0);

		for (Sort sort : limit.getSortSet().getSorts()) {
			if (sort.getOrder().equals(org.jmesa.limit.Order.DESC)) {
				criteria.addOrder(Property.forName(sort.getProperty()).desc());
			}
			else if (sort.getOrder().equals(org.jmesa.limit.Order.ASC)) {
				criteria.addOrder(Property.forName(sort.getProperty()).asc());
			}
		}

		List<T> data = criteria.list();
		logger.debug("Obtained " + data.size() + " results");
		return data;
	};
