...
<c:url value="/book/search/changePage.htm" var="changePage" scope="page" />
<c:url value="/book/search/sort.htm" var="changeSort" scope="page" />
...
<tt:errors name="bookSearchModel" />
<form:form method="post" action="doSearch.htm" modelAttribute="bookSearchModel" id="searchForm">
		...
		<td>
			<span id="cancelButton" class="yui-button yui-link-button">
    			<span class="first-child">
        			<a href="<c:url value='/book/list.htm'/>">Cancel</a>
    			</span>
			</span>
...
<table class="dataTable">
	...
	<c:forEach items="${books.data}" var="w" varStatus="st">
            <tr class="${st.index % 2 == 0 ? 'odd' : 'even'}">
                ...
                <td>${w.cool}</td>
				<td><a href="<c:url value="/book/search/edit.htm"/>?bookId=${w.id}"><img src="<c:url value="/images/Sweetie/png-24/16-tag-pencil.png" />"/></a></td>
				<td><a href="<c:url value="/book/search/delete.htm"/>?bookId=${w.id}"><img src="<c:url value="/images/Sweetie/png-24/16-em-cross.png" />"/></a></td>
			</tr>
	</c:forEach>
	</tbody>
</table>
...
var autoCompletefunc = function(){
	...
	var url = contextName + '/api/getBookNames.htm';
	...
	var ac = new YAHOO.book.AutoComplete("name","myContainer", oDS);
	ac.minQueryLength = 3;
	ac.useShadow = true;
...
YAHOO.util.Event.onDOMReady(function(){
    ...
    submitButton = new YAHOO.book.Button("submitButton");
    cancelButton = new YAHOO.book.Button("cancelButton");


	calButtonBegin = new YAHOO.book.Button("calButtonBegin");
	calButtonEnd = new YAHOO.book.Button("calButtonEnd");

    calendarBegin = new YAHOO.book.Calendar("calBegin", {iframe:true, hide_blank_weeks:true });
    calendarBegin.inputField = YAHOO.util.Dom.get("beginInitialTime");
    
	calendarEnd = new YAHOO.book.Calendar("calEnd", {iframe:true, hide_blank_weeks:true });
    ...
    dialogBegin = new YAHOO.book.Dialog("containerBegin", {
	...
	dialogEnd = new YAHOO.book.Dialog("containerEnd", {
